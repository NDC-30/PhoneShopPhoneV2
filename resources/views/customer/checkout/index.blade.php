                ...
                <div class="form-card">
                    ...
                    <div class="field-row">
                        <div class="field">
                            ...
                            @error('receiver_name')<div class="err-text">{{ $message }}</div>@enderror
                        </div>
                        <div class="field">
                            <label>Số điện thoại</label>
                            <input type="text" name="receiver_phone"
                                   value="{{ old('receiver_phone', $user->phone) }}" placeholder="09xx xxx xxx">
                            @error('receiver_phone')<div class="err-text">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    ...
                    <div class="field-row three">
                        <div class="field">
                            ...
                            @error('province')<div class="err-text">{{ $message }}</div>@enderror
                        </div>
                        <div class="field">
                            <label>Quận / Huyện</label>
                            <input type="text" name="district" value="{{ old('district') }}" placeholder="Cầu Giấy">
                            @error('district')<div class="err-text">{{ $message }}</div>@enderror
                        </div>
                        <div class="field">
                            <label>Phường / Xã</label>
                            <input type="text" name="ward" value="{{ old('ward') }}" placeholder="Dịch Vọng">
                            @error('ward')<div class="err-text">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    ...
                    <div class="field">
                        ...
                        @error('shipping_address')<div class="err-text">{{ $message }}</div>@enderror
                    </div>
                ...
                <div class="summary">
                    ...
                    <div class="voucher-apply" style="margin-bottom:14px">
                        <div style="display:flex;gap:8px">
                            <input type="text" id="vcode" class="vinput"
                                   placeholder="Nhập mã giảm giá"
                                   value="{{ $voucher['code'] ?? '' }}"
                                   style="flex:1;padding:11px 13px;border:1px solid var(--line);border-radius:9px;background:var(--bg);font-size:14px">
                            <button type="button" class="btn btn-dark" onclick="applyVoucher()" style="padding:0 18px">Áp dụng</button>
                        </div>
                        <div id="vmsg" style="font-size:12.5px;margin-top:7px;font-weight:500;
                             color:{{ $voucher ? 'var(--ok)' : 'var(--muted)' }}">
                            @if($voucher) Đã áp mã {{ $voucher['code'] }} — giảm {{ number_format($voucher['discount'],0,',','.') }}₫ @endif
                        </div>
                    </div>
                    ...
                    <button type="submit" class="btn btn-dark btn-block" style="margin-top:18px">Đặt hàng</button>
...
async function applyVoucher(){
    ...
    if(!code){ msg.style.color = 'var(--muted)'; msg.textContent = 'Vui lòng nhập mã.'; return; }
