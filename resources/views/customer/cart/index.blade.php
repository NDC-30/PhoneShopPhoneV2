        ...
        <div class="right">
            ...
            <div class="line-total">
                ...
                <aside class="summary">
                    ...
                    @if ($voucher)
                        <div class="line"><span>Voucher ({{ $voucher['code'] }})</span><span
                                class="accent">−{{ number_format($voucher['discount'], 0, ',', '.') }}₫</span></div>
                    @endif
                    <div class="line"><span>Phí vận
                            chuyển</span><span>{{ $subtotal >= 5000000 ? 'Miễn phí' : number_format(30000, 0, ',', '.') . '₫' }}</span>
                    </div>
                    @php
                    ...
                    @endphp
                    <div class="line total"><span>Tổng cộng</span><span>{{ number_format($total, 0, ',', '.') }}₫</span>
                    </div>

                    @auth
                        <a href="{{ route('checkout.index') }}" class="btn btn-dark btn-block" style="margin-top:18px">Tiến
                            hành thanh toán</a>
                    @else
                        <a href="{{ route('customer.login') }}" class="btn btn-dark btn-block" style="margin-top:18px">Đăng
                            nhập để thanh toán</a>
                        <p style="text-align:center;color:var(--muted);font-size:13px;margin-top:12px">Bạn cần đăng nhập để đặt
                            hàng</p>
                    @endauth
                    <a href="{{ route('shop.index') }}" class="btn btn-line btn-block" style="margin-top:10px">Tiếp tục mua
                        sắm</a>
                </aside>
            </div>
        @endif
