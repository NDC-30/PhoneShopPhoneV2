...
<nav class="topbar d-flex justify-content-between align-items-center"
    style="padding: 15px 30px; background: #fff; border-bottom: 1px solid #dee2e6;">
    ...
    <div class="topbar-right d-flex align-items-center gap-4">
        ...
        <form action="{{ route('admin.search') }}" method="GET" class="search-box m-0 position-relative">
            <input type="text" name="keyword" class="form-control form-control-sm"
                ...
                placeholder="Tìm sản phẩm, mã đơn..." value="{{ request('keyword') }}" required>
            <i class="bi bi-search position-absolute"
                style="top: 50%; right: 12px; transform: translateY(-50%); color: #6c757d; pointer-events: none;"></i>
            <button type="submit" class="d-none"></button>
        </form>

        <div class="user-menu dropdown">
            <button class="btn border-0 bg-transparent d-flex align-items-center gap-2 p-0" data-bs-toggle="dropdown"
                aria-expanded="false">

                @if ($admin && $admin->avatar && file_exists(public_path('storage/' . $admin->avatar)))
                    <img src="{{ asset('storage/' . $admin->avatar) }}?v={{ time() }}"
                        class="rounded-circle shadow-sm"
                        style="width:38px;height:38px;object-fit:cover;border:2px solid #eef2ff;">
                @else
                    <div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                        style="width:38px;height:38px;font-weight:bold;font-size:16px;">
                        {{ mb_substr($admin->fullname ?? 'A', 0, 1) }}
                    </div>
                @endif
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 mt-3"
                ...
                <li>
                    <form action="/logout" method="POST">
                        @csrf
                        <button type="submit"
                            class="dropdown-item text-danger py-2 d-flex align-items-center border-0 bg-white w-100 text-start">
