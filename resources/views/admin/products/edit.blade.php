        ...
        <div class="row">
            <div class="col-md-8 mx-auto">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.products.update', $product->product_id ?? $product->id) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-4">
                                <label class="form-label fw-bold">
                                    Ảnh chi tiết sản phẩm (tối đa 4 ảnh)
                                </label>

                                <input type="file" name="images[]" class="form-control" multiple accept="image/*">

                                <small class="text-muted">
                                    Bạn có thể chọn nhiều ảnh (ảnh đầu tiên là ảnh chính)
                                </small>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="status" class="form-select">
                                    <option value="1" {{ $product->status == 1 ? 'selected' : '' }}>Hiển thị công khai
                                    </option>
                                    <option value="0" {{ $product->status == 0 ? 'selected' : '' }}>Ẩn sản phẩm
                                    </option>
                                </select>
                            </div>
